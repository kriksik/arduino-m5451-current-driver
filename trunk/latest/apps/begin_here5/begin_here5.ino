...
void loop(void)
{
  ...
  while (doTest)  // Test the board
  {
  IrReceiver ir;   
  unsigned long int code=0;
  byte b=0;
  Usb.print("Enter function to test:\n");

  if (validCmd) 
    {
      while ((code = ir.read())!=0)  // Clear the IR buffer
        {
        }
      while (Usb.available()) Usb.read(); // Clear the USB buffer
      validCmd = 0;
    }
    
  // Wait for input
  while (!Usb.available() && ((code = ir.read())==0))
    {
    }
  p("IR code: %lx\n", code);
  if (Usb.available()) b = Usb.read();
  validCmd =1;
  if ((b == 's') || (b == '1') || (code == 0x80002aaa))
    {
      SourceDriverDemo();
    
    }
  else if ((b == 'n') || (b == '2') || (code == 0x20008aaa))
    {
    sinks.setBrightness(255);
    // Start PWMing automatically at around 4000hz.  Generally you'd do this at the beginning of your sketch and leave it on the entire time, not at the beginning of a function.
    pwm.StartAutoLoop(4000);      
    for (int i=0;i<35;i++) 
      ...
      p("PWM index %d and %d\n",i, i+35);
      pwm.brightness[i] = Lightuino_MAX_BRIGHTNESS-1;
      pwm.brightness[i+35] = Lightuino_MAX_BRIGHTNESS-1;
      mydelay(200);
      pwm.brightness[i] = 0;
      pwm.brightness[i+35] = 0;
      mydelay(1);
      }
    pwm.StopAutoLoop();
    pwm.loop();
  ...
  else if ((b=='a')||(b=='3') || (code == 0x5142200))
    {
      LightSensorDemo();

    }
  else if ((b=='i')||(b=='4') || (code == 0x5148000))
    {
      IrDemo();
  ...
  else  // Nope it was not a good command
  {
    validCmd=0;
  }
...
void print(unsigned long int i,char format)
{
